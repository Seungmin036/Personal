...
//-------------------------------------------------------------
        ...
        Eigen::Vector3d startPos = tpos_d_.r();
        std::cout << "tpos_d success" << std::endl;
        Eigen::Vector3d offset; offset.setZero();

        if (currentT_ >= traj_start_ && currentT_ < traj_start_ + traj_duration_) {
            // 1. 위로 이동 (0,0,0 → 0,0,0.2)
            Eigen::Vector3d traj; traj << 0, 0, 0.2;
            offset = ((currentT_ - traj_start_) / traj_duration_) * traj;
        }
        else if (currentT_ >= traj_start_ + traj_duration_ && currentT_ < traj_start_ + 2 * traj_duration_) {
            // 2. 왼쪽으로 이동 (0,0,0.2 → 0,-0.2,0.2)
            Eigen::Vector3d traj; traj << 0, -0.2, 0;
            offset = Eigen::Vector3d(0, 0, 0.2) + ((currentT_ - (traj_start_ + traj_duration_)) / traj_duration_) * traj;
        }
        else if (currentT_ >= traj_start_ + 2 * traj_duration_ && currentT_ < traj_start_ + 3 * traj_duration_) {
            // 3. 아래로 이동 (0,-0.2,0.2 → 0,-0.2,0)
            Eigen::Vector3d traj; traj << 0, 0, -0.2;
            offset = Eigen::Vector3d(0, -0.2, 0.2) + ((currentT_ - (traj_start_ + 2 * traj_duration_)) / traj_duration_) * traj;
        }
        else if (currentT_ >= traj_start_ + 3 * traj_duration_ && currentT_ < traj_start_ + 4 * traj_duration_) {
            // 4. 오른쪽으로 이동 (0,-0.2,0 → 0,0,0)
            Eigen::Vector3d traj; traj << 0, 0.2, 0;
            offset = Eigen::Vector3d(0, -0.2, 0) + ((currentT_ - (traj_start_ + 3 * traj_duration_)) / traj_duration_) * traj;
        }
        else {
            // 궤적 종료 후 원위치
