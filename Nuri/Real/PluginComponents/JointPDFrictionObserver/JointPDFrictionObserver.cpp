#include "JointPDFrictionObserver.h"

template<typename ROBOT>
void JointPDFrictionObserver<ROBOT>::initialize(ROBOT &robot, double delt)
{
...
template<typename ROBOT>
void JointPDFrictionObserver<ROBOT>::reset(ROBOT& robot)
{
    /**< Reset the integral terms in the H-inifinity control algorithm */
    robot.resetHinfController();
...
template<typename ROBOT>
void JointPDFrictionObserver<ROBOT>::compute(ROBOT &robot, const LieGroup::Vector3D &gravDir,
                                               const MotionData &motionData, ControlData &controlData)
{
    ...
    // -------------------------------- JOINT CONTROLLER -------------------------------------------------------
        ...
        JointVec error_; error_ = e_nr;
        JointVec JointPos; JointPos = robot.q();
        JointVec JointPos_D; JointPos_D = motionData.motionPoint.qd;
...
POCO_BEGIN_MANIFEST(NRMKControl::ControlAlgorithmCreator)
        POCO_EXPORT_CLASS(JointPDFrictionObserverCreator)
POCO_END_MANIFEST
