                                               ...
                                               const MotionData &motionData, ControlData &controlData)
{
    ...
    // --------------------------------- TASK CONTORLLER -------------------------------
        ...
        JointVec tau_rcm;
        cmd_ =  static_cast<int>(PW_[1]);
        //lambda_ = std::static_cast<int>(PW_[2]);
        state_transition(robot, gravDir, motionData, controlData);

        switch(current_state_)
        {
            case 0:
                tau_rcm = released_PD(robot, gravDir, motionData, controlData);
                break;
            case 1:
                tau_rcm = total_fixed_PD(robot, gravDir, motionData, controlData);
                break;
            case 2:
                tau_rcm = dot_fixed_PD(robot, gravDir, motionData, controlData);
                break;
            case 3:
                tau_rcm = line_fixed_PD(robot, gravDir, motionData, controlData);
                break;
        }
        ...
        std::cout << "est friction: " << friction_compensation_torque.transpose() << std::endl;
...
DemoExtremeHighGainFrictionObserver<ROBOT>::dot_fixed_PD(ROBOT &robot, const LieGroup::Vector3D &gravDir, const MotionData &motionData, ControlData &controlData)
{
    robot.computeFK(tpos_, tvel_);
    robot.computeJacobian(tpos_, tvel_, J_, Jdot_);

    Eigen::MatrixXd J_rcm; J_rcm.resize(6, 3); J_rcm.setZero();
    J_rcm = J_.bottomRows(3);

    ExtendedTaskVec e, edot;
    e.setZero(); edot.setZero();
    ...
    Eigen::MatrixXd J_rcm_inv;
    J_rcm_inv = J_rcm.transpose()*qr.inverse(); 
    tau_rcm = J_rcm.transpose() * ( (Kp_task_ * e).tail(3) + (Kd_task_ * edot).tail(3) )  + gravity_compensation_torque;
...
template<typename ROBOT>
typename DemoExtremeHighGainFrictionObserver<ROBOT>::JointVec
DemoExtremeHighGainFrictionObserver<ROBOT>::line_fixed_PD(ROBOT &robot, const LieGroup::Vector3D &gravDir, const MotionData &motionData, ControlData &controlData)
{
    JointVec gravity_compensation_torque = robot.tau();
    return gravity_compensation_torque + Kd_.cwiseProduct(- robot.qdot());
}

template<typename ROBOT>
typename DemoExtremeHighGainFrictionObserver<ROBOT>::JointVec
DemoExtremeHighGainFrictionObserver<ROBOT>::released_PD(ROBOT &robot, const LieGroup::Vector3D& gravDir, const MotionData &motionData, ControlData &controlData)
{
    JointVec gravity_compensation_torque = robot.tau();
    return gravity_compensation_torque + Kd_.cwiseProduct(- robot.qdot());
}
...
template<typename ROBOT>
typename DemoExtremeHighGainFrictionObserver<ROBOT>::JointVec
DemoExtremeHighGainFrictionObserver<ROBOT>::total_fixed_PD(ROBOT &robot, const LieGroup::Vector3D& gravDir, const MotionData &motionData, ControlData &controlData)
{
    JointVec gravity_compensation_torque = robot.tau();
    // Eigen::VectorXd kP_joint; kP_joint.resize(6); kP_joint.setConstant(Kp_task_vec_(3));
    // Eigen::VectorXd kD_joint; kD_joint.resize(6); kD_joint.setConstant(Kd_task_vec_(3));
    // std::cout << kP_joint.transpose() << "," << kD_joint.transpose() << std::endl;
    return gravity_compensation_torque + Kp_.cwiseProduct(fixed_joint_ - robot.q()) + Kd_.cwiseProduct(- robot.qdot());
}
...
template<typename ROBOT>
void DemoExtremeHighGainFrictionObserver<ROBOT>::state_transition(ROBOT &robot, const LieGroup::Vector3D& gravDir, const MotionData &motionData, ControlData &controlData)
{
    switch(current_state_)
    {
        case 0:
            ...
            {
                current_state_ = 1;
                fixed_joint_ = robot.q();
                std::cout << "STATE TRANSITION: RELEASE->SET"<<std::endl;
                std::cout << "Fixed Joint: "<< fixed_joint_.transpose()<<std::endl;
            }
            else if(cmd_ == 4)
            {
                current_state_ = 3;
                std::cout << "STATE TRANSITION: RELEASE->LINE FIXED"<<std::endl;
            }
            break;
        case 1:
            if(cmd_ == 9)
            {
                current_state_ = 0;
                std::cout << "STATE TRANSITION: SET->RELEASE"<<std::endl;
            }
            else if(cmd_ == 2)
            {
                ...
                {
                    current_state_ = 2;
                    std::cout << "STATE TRANSITION: SET->DOT FIXED || LAMBDA: "<<lambda_<<std::endl;
                }
                else
                {
                    std::cout << "LAMBDA OUT OF RANGE!!"<<std::endl;
                }
            }
            break;
        case 2:
            ...
            {
                current_state_ = 0;
                std::cout << "STATE TRANSITION: DOT FIXED->RELEASE"<<std::endl;
            }
            else if(cmd_ == 3)
            {
                current_state_ = 3;
                std::cout << "STATE TRANSITION: DOT FIXED->LINE FIXED"<<std::endl;
            }
            break;
        case 3:
            if(cmd_ == 9)
            {
                current_state_ = 0;
                std::cout << "STATE TRANSITION: LINE FIXED->RELEASE"<<std::endl;
            }
            else if(cmd_ == 5)
            {
                current_state_ = 1;
                fixed_joint_ = robot.q();
                std::cout << "Fixed Joint: "<< fixed_joint_.transpose()<<std::endl;
                std::cout << "STATE TRANSITION: LINE FIXED->SET"<<std::endl;
            }
            break;
        default :
            break;
