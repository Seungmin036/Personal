                                               ...
                                               const MotionData &motionData, ControlData &controlData)
{
    ...
    // -------------------------------- JOINT CONTROLLER -------------------------------------------------------
        ...
        controlData.controlReserved.qres2 = friction_compensation_torque;
    }
...
JointExtremeHighGainFrictionObserver<ROBOT>::friction_observer_L1_PD(ROBOT &robot, const LieGroup::Vector3D &gravDir, const JointVec &Control_input,  const MotionData &motionData, ControlData &controlData)  
{
    ...
    // if (currentT_ - printT_ > 1.0) {
    //     std::cout << "K_lpf: " << K_lpf_vec_.transpose() << std::endl;
    //     std::cout << "error: " << (robot.q() - motionData.motionPoint.qd).transpose() << std::endl;       
    //     printT_ = currentT_;
    // }
...
template<typename ROBOT>
void JointExtremeHighGainFrictionObserver<ROBOT>::sendStateUDP(ROBOT &robot){
    std::ostringstream ss;
    ss << "time," << currentT_;
    JointVec state_q = robot.q();
    JointVec state_qdot = robot.qdot();
    for (int i = 0; i < 6; ++i) {
        ss << ",q" << (i+1) << "," << state_q[i];
        ss << ",qdot" << (i+1) << "," << state_qdot[i];
    }
