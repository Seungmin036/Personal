...
void PhysicsLoop(mj::Simulate& sim) {
  ...
  Eigen::VectorXd q(6); q<<d->qpos[0],d->qpos[1],d->qpos[2],d->qpos[3],d->qpos[4],d->qpos[5];
  ...
  Eigen::Vector3d fixed_pt = pinocchio_data.oMf[pinocchio_model.getFrameId("nuri_4s_link6")].translation();
  Eigen::Vector3d fixed_vec = pinocchio_data.oMf[pinocchio_model.getFrameId("nuri_4s_link6")].rotation().col(2);

  Controller Controller(pinocchio_model);
  
  Eigen::VectorXd control_input; control_input.resize(6); control_input.setZero();
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (elapsedSim < 0 || elapsedCPU.count() < 0 || syncCPU.time_since_epoch().count() == 0 ||
            ...
            pinocchio::forwardKinematics(pinocchio_model,pinocchio_data,q);
            pinocchio::updateFramePlacements(pinocchio_model,pinocchio_data);
            control_input = Controller.computeLineFixedPD(m,d,fixed_pt,fixed_vec);
            // std::cout << fixed_pt.transpose() << "||"<<(pinocchio_data.oMf[pinocchio_model.getFrameId("nuri_4s_link6")].translation()).transpose() << std::endl;
            for(int i=0;i<6;i++)
            {
