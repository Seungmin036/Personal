...
def main():
    ...
    with open('simulation_log.csv', 'w', newline='') as csvfile:
        ...
        # 파일의 맨 첫 줄에 헤더(열 이름)를 작성합니다.
        csv_writer.writerow(['Time (s)', 'Error Norm'])
        
        # Get IK solution
        pre_grasp_q, pre_grasp_q2, rotate_eef_q_lst, rotate_eef_q_lst2 ,move_down_q_1_lst, move_down_q_2_lst = get_q_from_ik(env)
        ...
        while env.tick < max_tick:
            ...
            if env.tick % 1500 == 0:
                ...
                if task_sequnce_idx >= len(task_sequnce):
                    ...
                    if rot_idx < len(rotate_eef_q_lst) - 2:
                        rot_idx = rot_idx + 1
                    elif rot_idx == len(rotate_eef_q_lst2) - 2:
                        rot_idx = rot_idx + 1
                    else: break
            ...
            if current_task == "pre_grasp":
                ...
                desired_q = pre_grasp_q2
                desired_q = set_gripper(desired_q, option="close")

            elif current_task == "rotate_eef_i1_with_close":
                desired_q = rotate_eef_q_lst2[rot_idx+1]
                desired_q = set_gripper(desired_q, option="close")
            ...
            error_magnitude = np.linalg.norm(PID.err_curr)
                    
            csv_writer.writerow([current_time, error_magnitude])
