...
def get_q_from_ik(env):
    ...
    pre_grasp_p2 = np.array([0.78, 0.3, 1.4])
    ...
    for angle in to_rot_angle:
        ...
        move_down_p_2 = rotate_eef_p2 - np.array([-0.02, 0.0, 0.15])
    ...
    for rotate_eef_p2, rotate_eef_R in zip(rotate_eef_p_lst2, rotate_eef_R_lst):
        rotate_eef_q2 = solve_IK(
            env,
            max_tick=1000,
            p_trgt=rotate_eef_p2,
            R_trgt=rotate_eef_R,
            body_name=body_name,
            curr_q=pre_grasp_q2,
            is_render=is_render,
        ...
        rotate_eef_q_lst2.append(rotate_eef_q2)
    ...
    pre_grasp_q2 = np.concatenate([pre_grasp_q2, [np.pi, -np.pi]])
    for i, tmp in enumerate(rotate_eef_q_lst):
        tmp = np.concatenate([tmp, [np.pi, -np.pi]])
        rotate_eef_q_lst[i] = tmp
        
    for i, tmp in enumerate(rotate_eef_q_lst2):
        tmp = np.concatenate([tmp, [np.pi, -np.pi]])
        rotate_eef_q_lst2[i] = tmp
    ...
    return pre_grasp_q, pre_grasp_q2 ,rotate_eef_q_lst, rotate_eef_q_lst2, move_down_q_1_lst, move_down_q_2_lst
